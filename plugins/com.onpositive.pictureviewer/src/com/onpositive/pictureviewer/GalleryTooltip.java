...
extends DefaultToolTip {
    ...
    protected Image getImage(Event event) {
        ...
        if (item != null && item.getParent() != null) {
            IImageEntry data2 = (IImageEntry)item.getData();
            try {
                return ImageCache.getImage(item, data2);
            }
            catch (IOException e) {
                e.printStackTrace();
            }
    ...
    protected String getText(Event event) {
        ...
        if (item != null) {
            if (item.getParent() != null) {
                IImageEntry data2 = (IImageEntry)item.getData();
                try {
                    Image image2 = ImageCache.getImage(item, data2);
                    Rectangle bounds = image2.getBounds();
                    return String.valueOf(data2.getName()) + "(" + bounds.width + "," + bounds.height + ")";
                }
                catch (IOException e) {
                    e.printStackTrace();
